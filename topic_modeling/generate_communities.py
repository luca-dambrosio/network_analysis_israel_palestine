...
labels = {node: f'C{node}' for node in ind.nodes()}
# Adjusting label position to place them above the nodes
label_pos = {node: (pos_ind[node][0], pos_ind[node][1] + 0.1) for node in ind.nodes()}
nx.draw_networkx_labels(ind, labels=labels, pos=label_pos, font_size=16)
